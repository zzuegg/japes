# Tier-1 / tier-2 dispatch — when each is used
...
`skipReason` method rejects on five shapes:
...
### 5. Uses `@Filter(Removed)`

`@Filter(Added)` and `@Filter(Changed)` are expressible via the
per-component `ChangeTracker`'s dirty-list and tick comparisons,
so the generator emits them inline (the
`cachedFilterTrackers` loop in the generated preamble). But
`@Filter(Removed)` needs the removal log, which is an unbounded
out-of-band event stream, not a per-entity bitmap.

**Fix:** use `RemovedComponents<T>` as a service parameter instead.
It's the same signal — a list of entity ids whose `T` component
was dropped since the last tick — exposed as a service that
*does* stay on tier-1. `@Filter(Removed)` is effectively a
deprecated spelling.
...
exist because every realistic system shape the benchmark cross-
references comfortably fits inside them.
