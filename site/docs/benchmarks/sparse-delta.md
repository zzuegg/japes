...
|-----------|------------:|-----:|----------:|------:|---------:|--------:|
| `tick`    |       10000 | 4.11 |  **2.16** |  4.67 |     0.37 |    0.27 |
...
is enforced globally. At **2.16 µs/op japes is the fastest of the
library-change-detection group by a wide margin** (Bevy is 4.11,
Zay-ES is 4.67) after three rounds of profile-guided fixes (cached
...
dirty-bit fix from the code-review PR. **japes is 1.90× faster than
...
microbenchmark — Artemis is ~7× faster than japes, Dominion ~5×.
...
## What you're trading for that ~7× microbenchmark gap
...
!!! note "Fairness disclosure"
    ...
    comes in at 2.16 µs/op.

## Valhalla delta

| benchmark          | case | **japes** | **japes-v** | Δ            |
|--------------------|-----:|----------:|------------:|-------------:|
| `SparseDelta tick` |  10k |      2.16 |        1.96 | 1.10× faster |
